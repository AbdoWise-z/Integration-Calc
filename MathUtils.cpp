...
static long double parseTerm(){
    ...
    for (;;){
        if (eat('*')){
            ...
            if ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (ch == '(')){ //something like " 2sin(x) ---> 2 * sin(x)"
                x = x * parseFactor();
            }else return x;
        }
    }
...
static long double parseFactor() {
    ...
    if (eat('(')) { // parentheses
        ...
        if (charequal(func , func_sqrt , s1 , 4)){
            ...
            if (eat('(')){
                x = tan(parseExpression());
                eat(')');
            } else {
                x = tan(parseExpression());
            }
        }
        else if (charequal(func , func_asin , s1 , 4)){
            if (eat('(')){
                x = asin(parseExpression());
                eat(')');
            } else {
                x = asin(parseExpression());
            }
        }
        else if (charequal(func , func_acos , s1 , 4)){
            if (eat('(')){
                x = acos(parseExpression());
                eat(')');
            } else {
                x = acos(parseExpression());
            }
        }
        else if (charequal(func , func_atan , s1 , 4)){
            if (eat('(')){
                x = atan(parseExpression());
                eat(')');
            } else {
                x = atan(parseExpression());
            }
            ...
            if (eat('(')){
                x = 1.0 / tan(parseExpression());
                eat(')');
            } else {
                x = 1.0 / tan(parseExpression());
            }
        }
        else if (charequal(func , func_acsc , s1 , 4)){
            if (eat('(')){
                x = asin(1.0 / parseExpression());
                eat(')');
            } else {
                x = asin(1.0 / parseExpression());
            }
        }
        else if (charequal(func , func_asec , s1 , 4)){
            if (eat('(')){
                x = acos(1.0 / parseExpression());
                eat(')');
            } else {
                x = acos(1.0 / parseExpression());
            }
        }
        else if (charequal(func , func_acot , s1 , 4)){
            if (eat('(')){
                x = atan(1.0 / parseExpression());
                eat(')');
            } else {
                x = atan(1.0 / parseExpression());
            }
...
extern long double eval(const std::string& input){
    return eval((char*)input.c_str() , (int) input.length());
}
